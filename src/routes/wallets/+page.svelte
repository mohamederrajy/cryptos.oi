<script lang="ts">
    ...
    const cryptocurrencies: Cryptocurrency[] = [
        ...
        },
        {
            symbol: 'ETH',
            name: 'Ethereum',
            icon: 'data:image/svg+xml;base64,'
        },
        {
            symbol: 'BNB',
            name: 'Binance Coin',
    ...
    // Add these variables for deposits
    let deposits = [];
    let isLoadingDeposits = false;

    // Add this function to fetch deposits
    ...
    async function handleManualDeposit() {
        ...
        try {
            const response = await fetch(`${PUBLIC_API_URL}/api/deposit/manual`, {
                ...
                body: JSON.stringify({
                    amount: depositAmount,
                    currency: selectedCrypto.symbol.toLowerCase()
                })
            });
  
            if (response.ok) {
                const data = await response.json();
                notifications.success('Deposit submitted successfully');
                closeDepositModal();
                ...
                notifications.error(error.message || 'Failed to submit deposit');
            }
        } catch (error) {
            console.error('Deposit error:', error);
            notifications.error('Failed to process deposit');
        }
    }
    ...
    onMount(async () => {
        ...
        // Add this to onMount to load deposits initially
        await fetchDeposits();
    });
</script>
